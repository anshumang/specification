...
\apidescription{ 
    ...
    may be updated by \FUNC{shmem\_signal}, \ac{AMO} or \FUNC{shmem\_put\_signal} operation performed by another
    \ac{PE}.  In an \openshmem program with multithreaded \acp{PE}, the
    \VAR{ivar} object at the calling \ac{PE} may be updated by a \FUNC{shmem\_signal}, \ac{AMO} or
    \FUNC{shmem\_put\_signal} operation performed by a thread located within the calling \ac{PE} or
    within another \ac{PE}.

    \FUNC{shmem\_wait} and \FUNC{shmem\_wait\_until} should observe the update
    of the memory atomically when the update is done by \FUNC{shmem\_signal}, \ac{AMO} 
    or \FUNC{shmem\_put\_signal} operation.
